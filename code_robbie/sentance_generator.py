...
class SentanceGenerator:
    ...
    def wordlist(filename):
        f = open(filename, 'r')
        # wordlist = [SentanceGenerator.fixCaps(w) for w in re.findall(r"[\w']+|[.,!?;]", f.read())]
        wordlist = [SentanceGenerator.fixCaps(w) for w in re.findall(r"[\w]+|[.,!?;]", f.read())]
        f.close()
        return wordlist
    ...
    def addItemToTempMapping(self, history, word):
        while len(history) > 0:
            first = SentanceGenerator.toHashKey(history)
    ...
    def next(self, prevList):
        ...
        while SentanceGenerator.toHashKey(prevList) not in self.mapping:
            prevList.pop(0)
        # Get a random word from the mapping, given prevList
        for k, v in self.mapping[SentanceGenerator.toHashKey(prevList)].items():
            sum += v
            if sum >= index and retval == "":
                retval = k
        return retval

    def genSentence(self, markovLength, min_len = None, max_len = None):
        while True:
            ...
            if min_len is not None and len(sent) < min_len:
                continue
    ...
    def __init__(self, filenames, markovLength = 1):
        ...
        for filename in filenames:
            self.buildMapping(SentanceGenerator.wordlist(filename), markovLength)
